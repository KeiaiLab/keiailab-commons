...
| `pkg/status` | Stable | 4 標準 Condition Type + 6 Reason カタログ + ヘルパー (`SetReady`、`SetAvailable`、`SetReadyFalse`) + `UpdateWithRetry` (conflict リトライ付き status 永続化)。 |
...
| `pkg/apply` | Beta | 冪等 apply ヘルパー — `ConfigMap` / `Service` / `StatefulSet` / `Deployment` / `NetworkPolicy` / `PDB` / `HPA`、immutable フィールド + サーバーデフォルトを保持 (controller-runtime 依存)。 |
