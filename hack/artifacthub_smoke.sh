...
# Phase 6 ArtifactHub 등록 시 패키지 경로 확정
# (등록 name 에 따라 keiailab-commons 또는 keiailab-operator-commons 로 변경 가능)
...
helm_repo_url="${HELM_REPO_URL:-https://keiailab.github.io/operator-commons}"

curl_bin="${CURL_BIN:-curl}"
helm_bin="${HELM_BIN:-helm}"
jq_bin="${JQ_BIN:-jq}"

tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/operator-commons-artifacthub.XXXXXX")"
...
if [[ -z "$repo_json" ]]; then
	...
	echo "  fix: scripts/artifacthub-register.sh <uuid>" >&2
	exit 2
fi
