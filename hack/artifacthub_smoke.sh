...
artifacthub_repository_url="${EXPECTED_ARTIFACTHUB_REPOSITORY_URL:-${ARTIFACTHUB_REPOSITORY_URL:-oci://ghcr.io/keiailab/charts/keiailab-commons}}"
helm_repo_url="${HELM_REPO_URL:-https://keiailab.github.io/keiailab-commons}"
...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/keiailab-commons-artifacthub.XXXXXX")"
...
if [[ -n "$artifacthub_api_key_id" && -n "$artifacthub_api_key_secret" ]]; then
	repo_display_name="$("$jq_bin" -r '.display_name // "Keiailab Commons"' <<<"$repo_json")"
