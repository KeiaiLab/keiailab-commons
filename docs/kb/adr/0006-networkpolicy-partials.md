...
downstream operators 가 *서로 다른 두 NetworkPolicy 패턴* 을 보유:

| 패턴 | 출처 | 용도 |
|---|---|---|
| dataplane (default-deny + allow-internal-instance) | downstream operator chart templates/networkpolicy.yaml | managed workload (DB pod) 자체 보호 |
| controlplane (manager 자체 + metrics/webhook + API/DNS) | downstream operator chart templates/networkpolicy.yaml | operator manager pod 자체 보호 |
...
1. **`charts/keiailab-commons/templates/_networkpolicy.tpl` 신규**:
   - `keiailab.networkpolicy.dataplane` — dataplane 패턴 추출. caller 가
     fullname/labels/managedBy/port 를 dict 인자로 전달.
   - `keiailab.networkpolicy.controlplane` — controlplane 패턴 추출. caller 가
...
4. **PR 분할**:
   ...
   - 후속 PR (downstream operator): networkpolicy.yaml 신규 + dataplane partial include
     (해당 downstream operator 가 chart 에 networkpolicy 미보유 — 본 PR-B6 후 추가).
   - 후속 PR (downstream operators): 기존 networkpolicy.yaml → partial include
     로 교체.
...
- downstream operator 의 networkpolicy 미존재 차단 — 후속 PR 가 commons partial 로
  단순 추가 가능.
...
2. **partial 미추출, 각 chart 자체 유지** — 거부.
   ...
   - downstream operator chart 의 networkpolicy 부재 차단 path 부재.

## Refs

- Helm library chart 정책 §3.2.
- ADR-0005 (commons): §3.1 implementation.
- downstream operator chart templates/networkpolicy.yaml (dataplane reference).
- downstream operator chart templates/networkpolicy.yaml (controlplane reference).
- 구현 결정.
- 후속 downstream PRs: networkpolicy.yaml 신규 / 기존 → partial include.
