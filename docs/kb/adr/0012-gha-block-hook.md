# ADR-0012: GitHub Actions 차단 정책 GitHub Actions Block — lefthook pre-commit hook 자동 강제
...
| Author | keiailab |
| Supersedes | (none) |
| Related | GitHub Actions 차단 정책 (GitHub Actions Permanent Ban), ADR-0011 (lefthook consolidation) |

## Context

거버넌스 정책 GitHub Actions 차단 정책 (2026-04-29): "GitHub Actions 영구 금지 — `.github/workflows/` 디렉토리 추가 / 보존 **금지**".
...
gha-block:
  run: |
    ...
    if [ -n "$added" ]; then
      echo "❌ GitHub Actions 차단 정책 §2 위반: .github/workflows/ 신규 파일 추가 금지"
      echo "   파일: $added"
...
# 출력에 "GitHub Actions 차단 정책 §2 위반" + exit 1
