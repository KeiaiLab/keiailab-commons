# ADR-0004: pkg/version Generic Matrix[E] 추가 (downstream PR prerequisite)

- Date: 2026-05-09
- Status: Accepted
- Authors: @eightynine01
- Refs: 기존 downstream operator 의 `internal/version/matrix.go`
...
하여 downstream operator 의 `Combo` 같은 struct entry 도 commons 위임 가능하게 한다.
...
2. **`List` 와 공존**:
   - 기존 `List` API 무변경 — 단순 string 화이트리스트 사용자 영향 없음.
   - `Matrix[E]` 는 *추가 표면* — semver / channel / feature gate 같은
     메타데이터 필요한 downstream operator 만 사용.
...
- downstream operator 의 `internal/version/matrix.go` 가 commons `Matrix[Combo]` 로
  위임 가능 — *downstream consumer pkg/version 채택률 100%* (downstream PR 후).
- generic 으로 다양한 operator 의 *rich entry* 패턴 표준화 — downstream operator 의
  `Combo`-ish 진화 후속 가능.
...
- *commons 추가* (본 ADR) vs *downstream operator 자체 유지* — 후자는 downstream consumer
  cross-cut 변경 시 동기화 부담 (tooling unification 정책 §3.3 lint 위반).

## Alternatives Considered

1. **`Matrix[K comparable, V any]` (key + value 분리)** — 거부.
   - caller 의 `Combo` 가 *struct 자체가 entry* — key 분리 불필요.
...
- 기반 (downstream operator matrix.go → commons generic Matrix[Combo]).
- downstream operator `internal/version/matrix.go` (현 자체 구현).
- 후속 downstream PR: matrix.go 가 commons `Matrix[Combo]` 로 위임.
