...
// controllerutil 미의존 — finalizer slice 직접 조작 (commons 의 의존성
// 표면을 최소화). client.Update 은 호출자가 직접 수행 (본 패키지는
// *in-memory* 변경만).
package finalizer
...
func Add(obj metav1.Object, name string) bool {
	...
	if slices.Contains(finalizers, name) {
		return false
	}
...
func Remove(obj metav1.Object, name string) bool {
	finalizers := obj.GetFinalizers()
	idx := slices.Index(finalizers, name)
	if idx < 0 {
		return false
	}
...
func Has(obj metav1.Object, name string) bool {
	return slices.Contains(obj.GetFinalizers(), name)
}
