...
// goldenCases — keiailab-commons 골든 회귀 코퍼스. 각 케이스는 downstream operator 가
	...
	return []goldenCase{
		{
			...
			build: func() any {
				return NewServiceMonitor(ServiceMonitorParams{
					Name:      "downstream-app-metrics",
					Namespace: "default",
					Selector:  map[string]string{"app.kubernetes.io/name": "downstream-app"},
					Port:      "metrics",
				})
			},
		},
		{
			...
			build: func() any {
				return NewServiceMonitor(ServiceMonitorParams{
					Name:              "downstream-app-full-metrics",
					Namespace:         "data",
					Labels:            map[string]string{"app.kubernetes.io/part-of": "downstream-app", "team": "data"},
					Selector:          map[string]string{"app.kubernetes.io/name": "downstream-app"},
		...
		},
		{
			...
			build: func() any {
				return NewPrometheusRule(
					"downstream-app-alerts", "data",
					map[string]string{"app.kubernetes.io/instance": "downstream-app-cluster"},
					RuleGroup{
						Name:     "downstream-app.rules",
						Interval: "30s",
						Alerts: []AlertRule{
							{
								Alert:       "DownstreamAppDown",
								Expr:        `up{job="downstream-app"} == 0`,
								For:         "5m",
								Labels:      map[string]string{"severity": "critical"},
								Annotations: map[string]string{"summary": "downstream-app instance down"},
							},
						},
						Records: []RecordingRule{
							{
								Record: "downstream_app:up:ratio",
								Expr:   `avg(up{job="downstream-app"})`,
